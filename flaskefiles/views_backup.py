...
def prediction_advice(filename, title):
    filename = "/Users/yaoli/02_JobApplications/03_DataScience/insight/project/git_tracked/insight-project/web_app/flaskefiles/upload/" + filename
    
    #######################
    # input
    #video_title = "DRUGSTORE BACK TO SCHOOL MAKEUP TUTORIAL" # should be input
    #video_title = "Philadelphia"
    image_feature = compute_image_features(filename, title)
    ...
    if clf.predict(image_feature.reshape(1, -1)) == 1:
        ...
        message = "Oops! You might want to modify the thumbnail image and title of your video to optimize its impact. And here are what you can do to improve it..."
    return message
